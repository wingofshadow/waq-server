...
class MController extends \common\controllers\BaseController
{
    ...
    public function beforeAction($action)
    {
        ...
        // 不需要RBAC判断的路由全称
        $noAuthRoute = Yii::$app->params['noAuthRoute'];
        // 不需要RBAC判断的方法
        $noAuthAction = Yii::$app->params['noAuthAction'];
        ...
        if (!Yii::$app->user->isGuest) {
            $logRoute = [
                'sys/manager/personal',
                'sys/manager/up-passwd',
            ];
            if (in_array($permissionName, $logRoute)) {
                return true;
            }
        }
